...
def delete_bulk(
    ...
    """Hapus beberapa unit berdasarkan daftar ID."""
    if not ids:
        raise HTTPException(status_code=400, detail="Daftar ID tidak boleh kosong")
    deleted = (
        db.query(MonitoringData)
        .filter(MonitoringData.id.in_(ids))
        .delete(synchronize_session=False)
    )
    db.commit()
    return {"message": f"{deleted} data unit berhasil dihapus"}
...
def delete_all_units(
    ...
    """Hapus seluruh data unit (hanya manager)."""
    count = db.query(MonitoringData).count()
    db.query(MonitoringData).delete(synchronize_session=False)
    db.commit()
    return {"message": f"{count} data unit berhasil dihapus"}
