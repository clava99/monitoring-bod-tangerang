...
class MonitoringData(Base):
    ...
    pct_os_par = Column(Float)
    pct_os_lar = Column(Float)
    ao = Column(Integer)
    # APR 2026 (bulan sebelumnya untuk perbandingan)
    apr_noa = Column(Float)
    apr_noc = Column(Integer)
    apr_os_aktif = Column(Float)
    apr_lending = Column(Float)
    apr_noa_par = Column(Integer)
    apr_os_par = Column(Float)
    apr_noa_npl = Column(Integer)
    apr_os_npl = Column(Float)
    apr_os_3r = Column(Float)
    apr_noa_lar = Column(Integer)
    apr_os_lar = Column(Float)
    apr_pct_rr = Column(Float)
    # RKAP tambahan
    rkap_os_par = Column(Float)
    rkap_pct_os_npl = Column(Float)
    rkap_pct_os_lar = Column(Float)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
