...
function SigmonAnimation() {
  ...
  const live = tick % 20 < 10;

  const pts1 = "0,110 30,90 60,95 90,70 120,75 150,55 180,60 210,40 240,45 270,30 308,25";
  const pts2 = "0,130 30,120 60,125 90,110 120,115 150,100 180,108 210,95 240,100 270,88 308,85";

  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"1.5rem 1rem 1rem" }}>
      ...
      <p style={{ fontSize:11, color:"hsl(215,20%,55%)", marginBottom:12 }}>
        Sistem Informasi Monitoring BOD
      </p>

      <div style={{ width:300, height:195, borderRadius:12, background:"hsl(222,47%,10%)", border:"1.5px solid hsl(222,30%,22%)", overflow:"hidden", position:"relative" }}>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:30, borderBottom:"1px solid hsl(222,30%,18%)", display:"flex", alignItems:"center", padding:"0 12px", gap:6 }}>
          ...
          <span style={{ fontSize:9, color:"hsl(215,20%,50%)", marginLeft:4, fontFamily:"monospace" }}>SIGMON DASHBOARD</span>
          <span style={{ marginLeft:"auto", fontSize:9, color:"#4ecba0", fontFamily:"monospace", opacity: live ? 1 : 0.2 }}>LIVE</span>
        </div>

        <svg style={{ position:"absolute", top:34, left:10, width:280, height:122 }} viewBox="0 0 280 128">
          {[30,60,90,120].map(y => (
            <line key={y} x1="0" y1={y} x2="280" y2={y} stroke="hsl(222,30%,18%)" strokeWidth="0.5"/>
          ))}
          <polyline points={pts1} fill="none" stroke="#4ecba0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            transform={"translate(0," + offset + ")"}/>
          <polyline points={pts2} fill="none" stroke="#378add" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
            transform={"translate(0," + (-offset * 0.6) + ")"}/>
      ...
      <div style={{ display:"flex", gap:8, marginTop:10 }}>
        {[
          ...
          <div key={i} style={{ background:"hsl(222,47%,13%)", border:"0.5px solid hsl(222,30%,22%)", borderRadius:8, padding:"7px 12px", textAlign:"center", minWidth:76, transform:"translateY(" + Math.sin(tick*0.04 + i*1.8)*4 + "px)" }}>
      ...
      <div style={{ marginTop:10, fontSize:10, color:"#4ecba0", display:"flex", alignItems:"center", gap:5 }}>
        <span style={{ width:6, height:6, borderRadius:"50%", background:"#4ecba0", display:"inline-block", opacity: live ? 1 : 0.3 }}/>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:border-primary transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:border-primary transition-colors pr-10"
                />
            ...
            <button type="submit" disabled={loading}
              className="w-full py-2 px-4 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:opacity-90 transition-all disabled:opacity-60 flex items-center justify-center gap-2">
